...
void Sliders::add(const std::vector<SliderConfig>& configs)
{
    for (const auto& cfg : configs )
    { this->add(cfg); }
}

Sliders::void add(const SliderConfig& cfg);
{
    ...
    switch (cfg.type)
    {
        case SliderType::HORIZONTAL_SIMPLE:
            new_slider = std::make_unique<Slider>(
                cfg.start_pos, context_.graphics_manager, cfg.min_val, cfg.max_val, cfg.init_val, update_func
            );
            break;
        case SliderType::VERTICAL_SIMPLE:
            new_slider = std::make_unique<Slider>(
                cfg.start_pos, context_.graphics_manager, cfg.min_val, cfg.max_val, cfg.init_val, update_func
            );
            break;
    ...
    if ( new_slider )
    { sliders_.push_back(std::move(new_slider)) }   
}
